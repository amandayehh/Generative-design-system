...
figma.ui.onmessage = (message) => __awaiter(this, void 0, void 0, function* () {
    ...
    let red, green, blue, accent, backgroundRed, backgroundGreen, backgroundBlue, randFont, textFill, backgroundFill, buttonText, cardFill, shadowFill, cardRadius, cardRound, shadowSize, cardCompleteRound, buttonCompleteRound, buttonRadius, strokeWeight, cardOutline, accentH, lightH, lightS, lightB, lightFill, ultraLightFill, ultraLightS, ultraLightH, ultraLightB, hue, darkH, darkS, darkB, darkfill, accentS, accentB, accentFill, darkWhite, darkBlackH, darkBlackS, darkBlackB, darkgreyH, darkgreyS, darkgreyB, shadowH, shadowS, shadowB, shadowOpacity, strokeOpacity, lineFill;
    //GENERATE COLOR
    ...
    if (textFillLock.locked == false) {
        for (let node of figma.currentPage.findAll((n) => n.type === "TEXT" && n.name != "buttonText")) {
            if (node.type === "TEXT") {
                node.fills = [
                    {
                        type: "SOLID",
    ...
    if (typefaceLock.locked == false) {
        for (let node of figma.currentPage.findAll((n) => n.type === "TEXT")) {
            if (node.type === "TEXT" && "fontName" in node) {
                let fontStyle = Object.keys(node.fontName).map((key) => node.fontName[key])[1];
